    ...
    for i in [18, 28, 38, 48, 58, 68, 78, 88]:
        if pieceDict[i] == "wd":
            ...
            while newPiece not in ["q", "n", "r", "b"]:
                newPiece = input(
                    "What piece would you like to promote your pawn to? Type \"q\" for queen, \"k\" for knight, \"r\" for rook, and \"b\" for bishop.").replace(
                    "k", "n")
...
def imgPlace(key, Xcoor, Ycoor):
    ...
    pieceImg = {"p": [47, 57, 46, 56, 35, 45, 55, 65, 44, 54, 43, 53, 32, 42, 52, 62, 21, 31, 41, 51, 61, 71],
                ...
                "o": [],
                "r": [21, 31, 41, 51, 61, 71, 22, 32, 42, 52, 62, 72, 33, 43, 53, 63, 34, 44, 54, 64, 35, 45, 55, 65,
                      26, 36, 46, 56, 66, 76, 27, 47, 57, 77],
                "b": [47, 57, 46, 36, 35, 45, 55, 65, 44, 54, 43, 53, 32, 42, 52, 62, 21, 31, 41, 51, 61, 71],
                "n": [47, 57, 36, 46, 56, 66, 25, 35, 45, 55, 65, 24, 34, 44, 54, 64, 43, 53, 63, 32, 42, 52, 62, 21,
                      31, 41, 51, 61, 71],
                "q": [47, 57, 26, 46, 56, 76, 25, 35, 45, 55, 65, 75, 34, 44, 54, 64, 43, 53, 32, 42, 52, 62, 21, 31,
                      41, 51, 61, 71],
                "k": [21, 31, 41, 51, 61, 71, 32, 42, 52, 62, 23, 33, 43, 53, 63, 73, 24, 34, 44, 54, 64, 74, 45, 55,
                      36, 46, 56, 66, 47, 57]}
    ...
    for i in [-2, -1, 1, 2]:
        ...
        for i in [-2, -1, 1, 2]:
            ...
            if abs(addX) + abs(addY) == 3:
                if not ((str(int(x) + addX) in ["0", "-1", "-2", "9", "10", "11"]) or (
                        str(int(y) + addY) in ["0", "-1", "-2", "9", "10", "11"])):
...
def move(color):
    ...
    while valid != 1:
        tile = int(
            input("Which piece would you like to move, " + color.replace("w", "white").replace("b", "black") + "? "))
        try:
            pieceDict[int(tile)]
        except:
...
def play():
    game = input(
        "write a file name here. This will be your game's save file. If the file you type already exists, that file will be loaded. ")
    try:
        openFile(game)
        overwrite = input(
            "it looks like this game exists already. Would you like to overwrite it? Type \"1\" to open it and then overwrite, \"2\" to open it and leave it alone, and \"3\" to start a fresh game with this filename.")
    except:
        saveFile(game)
        overwrite = "1"
    if overwrite == "3":
        global pieceDict
        pieceDict = {11: 'wr1', 12: 'wp', 13: 'no', 14: 'no', 15: 'no', 16: 'no', 17: 'bp', 18: 'br1', 21: 'wn',
                     22: 'wp', 23: 'no', 24: 'no', 25: 'no', 26: 'no', 27: 'bp', 28: 'bn', 31: 'wb', 32: 'wp', 33: 'no',
                     34: 'no', 35: 'no', 36: 'no', 37: 'bp', 38: 'bb', 41: 'wq', 42: 'wp', 43: 'no', 44: 'no', 45: 'no',
                     46: 'no', 47: 'bp', 48: 'bq', 51: 'wk1', 52: 'wp', 53: 'no', 54: 'no', 55: 'no', 56: 'no',
                     57: 'bp', 58: 'bk1', 61: 'wb', 62: 'wp', 63: 'no', 64: 'no', 65: 'no', 66: 'no', 67: 'bp',
                     68: 'bb', 71: 'wn', 72: 'wp', 73: 'no', 74: 'no', 75: 'no', 76: 'no', 77: 'bp', 78: 'bn',
                     81: 'wr1', 82: 'wp', 83: 'no', 84: 'no', 85: 'no', 86: 'no', 87: 'bp', 88: 'br1', 99: '1'}
    printBoard()
    print(
        """welcome to chess. If you have not already, please run this code in the terminal of your device. click "run" > "run current script in terminal". You should then resize the terminal to see the chessboard, as you will be zoomed in and somewhat distorted by default. Holding ctrl and scrolling will zoom the terminal window. Note that visual errors will occur if your terminal is in "fullscreen" mode. Press Enter when you are ready to begin.""")
    input("")
